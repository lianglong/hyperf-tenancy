...
class DatabaseConfig
{
    ...
    public static function __constructStatic(): void
    {
        ...
        static::$passwordGenerator = static::$passwordGenerator ?? function (Tenant $tenant) {
            return password_hash(Str::random(32), PASSWORD_BCRYPT, [
                    'cost' => 10,
                ]);
//            return Hash::make(Str::random(32));
    ...
    public function getTemplateConnectionName(): string
    {
        ...
        if( $this->tenant->getInternal('db_connection') ){
            $internalConnection = $this->tenant->getInternal('db_connection');
        }elseif ( $config = $this->tenantConfig() ){
            $central_connection = config('tenancy.database.central_connection');
            if(
                ($config['host'] != '' && $config['host'] != config('databases.'.$central_connection.'.host')) ||
                ($config['username'] != '' && $config['username'] != config('databases.'.$central_connection.'.username'))
            ){
                $internalConnection = 'tenant'.$this->tenant->getTenantKey();
            }
        }
    ...
    public function connection(): array
    {
        ...
        $defaultConnectionName = config('tenancy.database.central_connection');
        $templateConnection = config("databases.{$template}") ?? config("databases.{$defaultConnectionName}");

        $databaseName = ( $this->tenant->getInternal('db_name') || $template != $defaultConnectionName ) ? $this->getName() : $templateConnection['database'];//config("databases.{$defaultConnectionName}.database");
        return $this->manager()->makeConnectionConfig(
            array_merge($templateConnection, $this->tenantConfig()), $databaseName
        );
    }
...
//        $driver = config("database.connections.{$this->getTemplateConnectionName()}.driver");
        $driver = config("databases.{$this->getTemplateConnectionName()}.driver") ?? config('databases.'.config('tenancy.database.central_connection').'.driver');
        ...
        $databaseManager = make($databaseManagers[$driver]);
