...
namespace Stancl\Tenancy\Virtualcolumn;
...
trait VirtualColumn
{
     ...
     *
     * @return null|object|\Psr\EventDispatcher\StoppableEventInterface
     */
    protected function fireModelEvent($event): ?object
    {
        $this->decodeIfEncoded();

        $result = parent::fireModelEvent($event);

        $this->runAfterListeners($event);

        return $result;
    }

    public function runAfterListeners($event)
    {
